...
- **问题**：真实 Run 曾在 `agent_started` 与下一条阶段事件之间出现 6–15 分钟空窗，外部监督器无法
  区分正常推理、长工具调用和进程停滞。
- **解决**：Round Clock 默认每 60 秒发出 `agent_heartbeat`，只包含单调序号、已用时、进程存活状态和
  Run/Round 标识，不包含模型内容、工具参数或 Development/Gate 指标。心跳不修改 deadline、预算、
  Decision、恢复语义或成功 Run 的耐久工件策略；间隔可通过 Round Clock 构造参数调整。
- **验证**：确定性时钟测试覆盖间隔边界、事件内容与阶段警告共存；原有 deadline 和 checkpoint 测试
  继续覆盖停止及恢复语义。
- **风险**：进程存活不等于 Provider 正在取得进展，因此心跳只用于观测，硬 deadline 仍是权威停止条件。
