...
def normalize_spot_frame(spot: pd.DataFrame) -> pd.DataFrame:
    ...
    frame = spot.rename(columns={key: value for key, value in aliases.items() if key in spot.columns}).copy()
    ...
    if "total_market_value" in frame.columns:
        total = pd.to_numeric(frame["total_market_value"], errors="coerce")
    else:
        total = pd.Series(pd.NA, index=frame.index, dtype="Float64")
    if "float_market_value" in frame.columns:
        floating = pd.to_numeric(frame["float_market_value"], errors="coerce")
    else:
        floating = pd.Series(pd.NA, index=frame.index, dtype="Float64")
